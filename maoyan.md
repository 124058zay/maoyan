# pytho爬虫实战之猫眼专业版数据实时爬取：

## 网址：[猫眼专业版-实时票房 (maoyan.com)](https://piaofang.maoyan.com/dashboard)

### 此网站的加密和反爬手段做的是非常高的，对爬取技术也是非常具有挑战的，我一会给大家讲一下大概的爬取思路。

![](E:\爬虫MD文档\猫眼专业版\1.png)
        ...
        for i in tmpstr:
            ...
            for k in range(len(uni_list)):
                if tmp == uni_list[k]:
                    ...
                    break
        print(
            f'{moviename[j]}\t{movieInfo[j]}\t{sumBoxDesc[j]}\t综合票房：{rstr}{enNumDw[j]}\t票房占比：{boxRate[j]}\t排片场次：{showCount[j]}')
